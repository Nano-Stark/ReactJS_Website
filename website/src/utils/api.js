...
if (process.env.NODE_ENV === 'development'){
    ...
    host = 'https://powerful-citadel-92824.herokuapp.com/';//host address
}
...
const API  = {
    ...
    register: (name, email, pass, success) => {
        ...
        .catch(err => {
            success(err);//used to catch error if it occurs
        })
    },
    ...
    getPostBySlug: (slug, token, success) => {
        axios.get(`${host}/api/Posts/findOne?access_token=${token}`, {
            params: {
                filter: {
                    where: {slug: slug}, 
                    include: {Comments: 'profile'}//includes {comment}, not likely to include tge comment, its prob a ref and the comment profile
                }
            }
        }).then(res => {
            success(res)
